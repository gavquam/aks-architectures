...
| --- | --- | --- |
| Be walked through the decisions and come out with a cluster | [Quick start](#quick-start) → `make wizard` | Not until you confirm the estimate |
| Skip the interview — you already know what you want | [Quick start](#quick-start) → [Or drive it directly](#or-drive-it-directly) | Not until you confirm the estimate |
| Evaluate the repository end to end before committing to anything | **[WALKTHROUGH.md](WALKTHROUGH.md)** | Most steps are free, and each one says which |

Seven architectures ship here, from a throwaway sandbox to a private cluster to Arc-attaching
clusters you already run elsewhere — [Choosing an architecture](#choosing-an-architecture) is the
decision table.
...
| **New enterprise build, hub-and-spoke, admins reach the cluster from on-premises** | **`aks-private-vnet-integration`** | **The model to adopt as the new enterprise standard.** The API server is injected into a delegated subnet in your VNet — same privacy as Private Link, and the nodes reach it without a DNS lookup at all, which is the thing that breaks most private clusters. AKS still creates and links the API server's DNS zone for you, so operators connecting from on-premises use it much as they would with Private Link. A cluster can also be converted between public and private without a rebuild. **Prefer this for anything new.** |
