// Architecture: aks-private-link
// The regulated / high-security shape. Private API server behind a private endpoint, DNS resolvable
// from on-premises, and optionally all egress forced through Azure Firewall.
//
// Egress defaults to natgateway, not udr-firewall. Azure Firewall is the single most expensive
// thing this repository can create - it bills per hour plus per GB whether or not traffic flows -
// so it is opt-in: set AKS_EGRESS=udr-firewall to deploy the firewall-forced-tunnelling variant
// that a regulated production environment would actually run. Everything else about the architecture is
// identical either way. scripts/pause.* can deallocate the firewall between demos.
//
// kubectl needs network line of sight to the private endpoint. Preflight check 7 verifies the
// private DNS zone is linked to the VNet you will connect from; if you connect over ExpressRoute
// or VPN, add that VNet to additionalVnetIdsToLink.
...
param architecture = 'aks-private-link'
param networkProfile = 'cni-overlay'
param egress = readEnvironmentVariable('AKS_EGRESS', 'natgateway')

param addressing = {
  vnetAddressSpace: '10.62.0.0/16'
  nodeSubnetPrefix: '10.62.0.0/22'
  systemNodeSubnetPrefix: ''
  podSubnetPrefix: ''
  apiServerSubnetPrefix: ''
  firewallSubnetPrefix: '10.62.17.0/26'
  bastionSubnetPrefix: '10.62.17.64/26'
  privateEndpointSubnetPrefix: '10.62.18.0/24'
  dnsResolverInboundPrefix: '10.62.19.0/28'
  dnsResolverOutboundPrefix: '10.62.19.16/28'
...
param firewallSkuTier = readEnvironmentVariable('AKS_FIREWALL_SKU', 'Standard')

// This architecture takes the tier as-is. At lean you get the private cluster, the private endpoint and
// the private DNS zones - the parts that make it a private-link cluster - without Bastion or the
// DNS Private Resolver, which are conveniences for reaching it and bill per hour. Use the full tier
// when you want the complete regulated shape.
