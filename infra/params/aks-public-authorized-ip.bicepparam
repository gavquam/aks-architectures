// Architecture: aks-public-authorized-ip
// Public API server restricted to an explicit allowlist. Set AKS_AUTHORIZED_IP_RANGES to the
// egress CIDRs your operators and CI runners actually come from, comma separated. The cluster's
// own egress address is appended automatically because egress is natgateway; without that the
...
var callerRanges = readEnvironmentVariable('AKS_AUTHORIZED_IP_RANGES', '')
...
param architecture = 'aks-public-authorized-ip'
param networkProfile = 'cni-overlay'
param egress = readEnvironmentVariable('AKS_EGRESS', 'natgateway')

param addressing = {
  vnetAddressSpace: '10.61.0.0/16'
  nodeSubnetPrefix: '10.61.0.0/22'
  systemNodeSubnetPrefix: ''
  podSubnetPrefix: ''
  apiServerSubnetPrefix: ''
  firewallSubnetPrefix: ''
  bastionSubnetPrefix: '10.61.17.64/26'
  privateEndpointSubnetPrefix: '10.61.18.0/24'
  dnsResolverInboundPrefix: '10.61.19.0/28'
  dnsResolverOutboundPrefix: '10.61.19.16/28'
...
// A deployment with an empty allowlist is rejected by the validation gate in main.bicep, which is
// deliberate: an empty allowlist on this architecture silently means "allow the world".
param authorizedIpRanges = empty(callerRanges) ? [] : split(callerRanges, ',')
...
// Nothing is forced here: this architecture is the general-purpose enterprise shape, so it takes the
// tier as-is except for the resolver, which only earns its hourly charge with a private cluster.
